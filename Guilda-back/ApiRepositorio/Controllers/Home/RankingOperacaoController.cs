...
{
    //[Authorize]
    public class RankingOperacaoController : ApiController
    {
        public List<ModelsEx.homeRel> returnHomeRanking(string dtInicial, string dtFinal, string sectors, string indicators, string hierarchies, string ordem, string collaborators, string indicatorsAccessAsString)
        {
            //PREPARAR OS FILTROS
            string filter = "";
            string orderBy = "";
        ...
        public IHttpActionResult PostResultsModel([FromBody] InputModel inputModel)
        {
            ...
            List<ModelsEx.homeRel> rmams = new List<ModelsEx.homeRel>();
            rmams = returnHomeRanking(dtInicial, dtFinal, sectorsAsString, indicatorsAsString, hierarchiesAsString, inputModel.Order,  filtro, indicatorsAcessoAsString);

            //Adicionando os indicadores de taxa de acesso
            if (indicatorsAcessoAsString != "" || (indicatorsAsString == "" && indicatorsCestaAsString == ""))
            {
                List<ModelsEx.homeRel> listaIndicadorAcesso = new List<ModelsEx.homeRel>();
                listaIndicadorAcesso = Funcoes.retornaIndicadorAcesso(dtInicial, dtFinal, sectorsAsString, "", "", inputModel.Order);
            ...
            List<ModelsEx.homeRel> resultList = hierarchies.GroupBy(d => new { d.idcollaborator, d.indicador }).Select(item => new ModelsEx.homeRel
            {
                ...
                sumDiasLogadosEscalados = item.Count(item2 => item2.sumDiasEscalados == 1 && item2.sumDiasLogados == 1),
            }).ToList();
