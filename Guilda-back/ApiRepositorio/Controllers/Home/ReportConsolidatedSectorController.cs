...
{
    //[Authorize]
    public class ReportConsolidatedSectorController : ApiController
    {
        ...
        public static List<returnResponseConsolidatedSetor> relConsolidatedSector(string dtInicial, string dtFinal, string sectorsAsString, string indicatorsAsString, string indicatorsCestaAsString, bool consolidated, string orderFil, string indicatorsAccessAsString, bool Thread = false)
        {
            ...
            if (indicatorsAccessAsString != "" || (indicatorsAsString == "" && indicatorsCestaAsString == ""))
            {
                ...
                listaIndicadorAcesso = Funcoes.retornaIndicadorAcesso(dtInicial, dtFinal, sectorsAsString, "", "", orderFil, Thread);
            ...
            List<ModelsEx.homeRel> rmamsTeste = agrupadoSetor.FindAll(y => y.cod_indicador == "598").ToList();
            ...
            var jsonData = agrupadoSetor.Select(item => new returnResponseConsolidatedSetor
            {
                ...
                Meta = item.indicatorType == "HOUR" ? ResultConsolidatedController.FormatTime(item.meta.ToString().Replace(".", ",")) : item.meta.ToString().Replace(".", ","),
                Resultado =  Math.Round(item.resultado, 2),
                ResultadoHora = item.indicatorType == "HOUR" ? ResultConsolidatedController.FormatTime(Math.Round(item.resultado).ToString()) : "",
        ...
        public class returnResponseConsolidatedSetor
        {
            ...
            public string  ResultadoHora { get; set; }
        ...
        public IHttpActionResult PostResultsModel([FromBody] InputModel inputModel)
        {
            ...
            //}
            var jsonData = relConsolidatedSector(dtInicial, dtFinal, sectorsAsString, indicatorsAsString, indicatorsCestaAsString, consolidated, indicatorsAcessoAsString, order);
            return Ok(jsonData);
