...
{
    //[Authorize]
    public class ReportHomeResultController : ApiController
    {
        public static int returnHomeResultQtd(string dtInicial, string dtFinal, string sectors, string indicators, string hierarchies, string ordem, bool Thread = false)
        {
            int qtd = 0;
            //PREPARAR OS FILTROS
            string filter = "";
        ...
        public static int relHomeResultQtd(string dtInicial, string dtFinal, string groupsAsString, string sectorsAsString, string hierarchiesAsString, string indicatorsAsString, string indicatorsCestaAsString, string CollaboratorId, string orderFil, string indicatorsAccessAsString, bool Thread = false)
        {

            int qtd = 0;
            qtd = returnHomeResultQtd(dtInicial, dtFinal, sectorsAsString, indicatorsAsString, hierarchiesAsString, orderFil, Thread);

            //Adicionando os indicadores de taxa de acesso
            if (indicatorsAccessAsString != "" || (indicatorsAsString == "" && indicatorsCestaAsString == ""))
            {

                int qtd2 = Funcoes.retornaIndicadorAcessoQtd(dtInicial, dtFinal, sectorsAsString, CollaboratorId, hierarchiesAsString, orderFil, Thread);
                qtd = qtd + qtd2;
            }
        ...
        public static List<returnResponseHome> relHomeResult(string dtInicial, string dtFinal, string groupsAsString, string sectorsAsString, string hierarchiesAsString, string indicatorsAsString, string indicatorsCestaAsString, string CollaboratorId, string orderFil, string indicatorsAccessAsString, bool Thread = false)
        {
            ...
            if (indicatorsAccessAsString != "" || (indicatorsAsString == "" && indicatorsCestaAsString == ""))
            {
                ...
                listaIndicadorAcesso = Funcoes.retornaIndicadorAcesso(dtInicial, dtFinal, sectorsAsString, CollaboratorId, hierarchiesAsString, orderFil, Thread);
            ...
            var jsonData = elementosFiltrados.Select(item => new returnResponseHome
            {
                ...
                Resultado = item.indicatorType == "HOUR" ? ResultConsolidatedController.FormatTime(item.resultado.ToString().Replace(".", ",")) : item.resultado.ToString().Replace(".", ","),
                Meta = item.indicatorType == "HOUR" ? ResultConsolidatedController.FormatTime(item.meta.Replace(".", ",")) : item.meta.Replace(".", ","),
        ...
        public IHttpActionResult PostResultsModel([FromBody] InputModel inputModel)
        {
            ...
            int qtdLinhas = relHomeResultQtd(dtInicial, dtFinal, groupsAsString, sectorsAsString, hierarchiesAsString, indicatorsAsString, indicatorsCestaAsString, CollaboratorId, order, indicatorsAcessoAsString);

            //Caso não seja selecionado um setor
            if (sectorsAsString == "" && qtdLinhas > 500000)
            {
                string NameFile = $"Relatorio_Resultado {inputModel.dataInicial.ToString("MM")}-{inputModel.dataInicial.ToString("yyyy")}.txt";

                //var blobInfo= BucketClass.GetFile(NameFile);
                //return Ok(blobInfo);
                List<returnResponseHome> asds = new List<returnResponseHome>();
                returnResponseHome asd = new returnResponseHome();
                asd.TypeFile = "";
                asd.FileName = "";
                asds.Add(asd);
                var jsonData2 = asds.Select(item => new returnResponseHome
                {
                    FileName = NameFile,
                    TypeFile = "TXT",
                }).ToList();
            ...
            //}
            var jsonData = relHomeResult(dtInicial, dtFinal, groupsAsString, sectorsAsString, hierarchiesAsString, indicatorsAsString, indicatorsCestaAsString, CollaboratorId, order, indicatorsAcessoAsString);

            //Retirar Tx de Acesso
            jsonData = jsonData.Where(rd => rd.CodigoIndicador != "-1").ToList();
