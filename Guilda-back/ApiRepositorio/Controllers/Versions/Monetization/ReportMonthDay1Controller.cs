...
{
    //[Authorize]
    public class ReportMonthDay1Controller : ApiController
    {
        ...
        public List<ModelsEx.homeRel> returnMonetizationDayMonth(string dtInicial, string dtFinal, string sectors, string indicators, string collaborators, string hierarchies, string ordem)
        {
            ...
            if (sectors != "")
            {
                ...
                filter2 = filter2 + $" AND CL2.IDGDA_SECTOR IN ({sectors}) ";
            }
            //if (groups != "")
            //{
            //    filter = filter + " ";
            ...
            if (indicators != "")
            {
                ...
                filter2 = filter2 + $" AND R.INDICADORID IN ({indicators}) ";
            }
            if (hierarchies != "")
            {
                filter = filter + $" AND CL.IDGDA_HIERARCHY IN ({hierarchies}) ";
                filter2 = filter2 + $" AND CL2.IDGDA_HIERARCHY IN ({hierarchies}) ";
            }
            if (collaborators != "")
            {
                filter = filter + $" AND R.IDGDA_COLLABORATORS IN ({collaborators}) ";
                filter2 = filter2 + $" AND R.IDGDA_COLLABORATORS IN ({collaborators}) ";
            }
            //if (ordem != "")
            //{
            //    if (ordem.ToUpper() == "MELHOR")
            //    {
            //        orderBy = " ORDER BY R.INDICADORID, MAX(MZ.INPUT) DESC ";
            //    }
            //    else
            //    {
            //        orderBy = " ORDER BY R.INDICADORID, MAX(MZ.INPUT) ASC ";
            //    }
            ...
            stb.Append(" AND ((CL.IDGDA_SECTOR IS NULL OR CL.CARGO IS NULL OR CL.HOME_BASED = '') OR CL.ACTIVE IS NULL) ");
            stb.Append("  AND R.DELETED_AT IS NULL ");
            ...
            //stb.Append("AND MZ.result_date = R.CREATED_AT ");
            //stb.Append("AND MZ.COLLABORATOR_ID = R.IDGDA_COLLABORATORS ");
            //stb.Append("INNER JOIN GDA_BASKET_INDICATOR AS BK ON BK.SECTOR_ID = HCS.IDGDA_SECTOR "); //Coloquei inner para retornar apenas os indicadores monetizaveis
            //stb.Append("AND BK.INDICATOR_ID = R.INDICADORID ");
            //stb.Append("AND BK.DATE = R.CREATED_AT ");
        ...
        public List<ModelsEx.homeRel> returnListHierarchy(List<ModelsEx.homeRel> original, string hierarchy)
        {
            ...
            try
            {
                ...
                {
                    retorno = original
                        .GroupBy(item => new { item.cod_indicador, item.dateReferer, item.matricula_supervisor }).Where(d => d.Key.matricula_supervisor != "-")
                ...
                {
                    retorno = original
                    .GroupBy(item => new { item.cod_indicador, item.dateReferer, item.matricula_coordenador }).Where(d => d.Key.matricula_coordenador != "-")
                    .Select(grupo => new ModelsEx.homeRel
                    {
                        datePay = grupo.First().datePay,
                        dateReferer = grupo.First().dateReferer,
                ...
                {
                    retorno = original
                    .GroupBy(item => new { item.cod_indicador, item.dateReferer, item.matricula_gerente_ii }).Where(d => d.Key.matricula_gerente_ii != "-")
                    .Select(grupo => new ModelsEx.homeRel
                    {
                        datePay = grupo.First().datePay,
                        dateReferer = grupo.First().dateReferer,
                ...
                {
                    retorno = original
                    .GroupBy(item => new { item.cod_indicador, item.dateReferer, item.matricula_gerente_i }).Where(d => d.Key.matricula_gerente_i != "-")
                    .Select(grupo => new ModelsEx.homeRel
                    {
                        datePay = grupo.First().datePay,
                        dateReferer = grupo.First().dateReferer,
                ...
                {
                    retorno = original
                    .GroupBy(item => new { item.cod_indicador, item.dateReferer, item.matricula_diretor }).Where(d => d.Key.matricula_diretor != "-")
                    .Select(grupo => new ModelsEx.homeRel
                    {
                        datePay = grupo.First().datePay,
                        dateReferer = grupo.First().dateReferer,
                ...
                {
                    retorno = original
                    .GroupBy(item => new { item.cod_indicador, item.dateReferer, item.matricula_ceo }).Where(d => d.Key.matricula_ceo != "-")
                    .Select(grupo => new ModelsEx.homeRel
                    {
                        datePay = grupo.First().datePay,
                        dateReferer = grupo.First().dateReferer,
        ...
        public IHttpActionResult PostResultsModel([FromBody] InputModel inputModel)
        {
            ...
            for (int i = 0; i < listaAgentes.Count; i++)
            {
                ...
                if (listaAgentes[i].idcollaborator == "753027" && listaAgentes[i].cod_indicador == "371")
                {
                    var parou = true;
                }
                ...
                //if (agente.idcollaborator == "785363" && agente.cod_indicador == "73")
                //{
                //    var parou = true;
                //}
            ...
            ModelsEx.homeRel teste = listaCesta.Find(l => l.name == "PATRICIA LIMA NOGUEIRA");
            ...
            var tst = elementosFiltrados[0].dateReferer;
            ...
            var jsonData = elementosFiltrados.Select(item => new returnResponseDay
            {
                ...
                Percentual = item.cargo == "AGENTE" || item.cod_indicador == "10000012" ? item.porcentual.ToString() : "-",
                MetaMaximaMoedas = item.moedasPossiveis.ToString(),
                GanhoEmMoedas = item.moedasGanhas,
                Grupo = item.cargo == "AGENTE" || item.cod_indicador == "10000012" ? item.grupo.ToString() : "-",
