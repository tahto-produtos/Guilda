...
{
    //[Authorize]
    public class ReportMonthConsolidated2Controller : ApiController
    {
        public static List<RelMonetizationAdmMonth> returnMonetizationConsolidated(string dtInicial, string dtFinal, string sectors, string collaborators, string hierarchies, string ordem, bool Thread = false)
        {
            // Preparar Filtros
            string filter = "";
            string orderBy = "";
            ...
            stb.Append("LEFT JOIN (SELECT  IDGDA_COLLABORATORS,  ");
            stb.Append("				   CARGO,  ");
            stb.Append("				   CASE WHEN IDGDA_SUBSECTOR IS NULL THEN IDGDA_SECTOR  ");
            stb.Append("				   ELSE IDGDA_SUBSECTOR END AS IDGDA_SECTOR,  ");
            ...
            stb.Append("				   FROM GDA_COLLABORATORS_DETAILS (NOLOCK)) as CL ON CL.IDGDA_COLLABORATORS = CB.IDGDA_COLLABORATORS AND CL.CREATED_AT = CONVERT(DATE,CA.RESULT_DATE)  ");
       ...
       public static List<returnResponseConsolidated> RelSaldoAcumulado(string dtInicial, string dtFinal, string sectorsAsString, string hierarchiesAsString, string colaboratorsAsString, string order, bool Thread = false)
        {
            ...
            List<RelMonetizationAdmMonth> rmams = new List<RelMonetizationAdmMonth>();
            rmams = returnMonetizationConsolidated(dtInicial, dtFinal, sectorsAsString, colaboratorsAsString, hierarchiesAsString, order, Thread);

            if (order != "")
            {
                if (order.ToUpper() == "MELHOR")
                {
        ...
        public IHttpActionResult PostResultsModel([FromBody] InputModel inputModel)
        {
            ...
            {
                BadRequest("Selecionar uma data de no maximo 1 mês!");
            }

            var jsonData = RelSaldoAcumulado(dtInicial, dtFinal, sectorsAsString, hierarchiesAsString, colaboratorsAsString, order);
