...
{
    //[Authorize]
    public class ReportMonthAdm1Controller : ApiController
    {
        public List<ModelsEx.homeRel> returnMonetizationAdmMonth(string dtInicial, string dtFinal, string sectors, string indicators, string collaborators, string hierarchies, string ordem)
        {

            List<colaboradoresTrabalhados> lColaboradoresTrabalhados = returnTables.colaboradoresTrabalhados(dtInicial, dtFinal);

            //lColaboradoresTrabalhados = lColaboradoresTrabalhados.GroupBy(t => t.)
            List<colaboradoresEscalados> lColaboradoresEscalados = returnTables.colaboradoresEscalados(dtInicial, dtFinal);
            ...
            //stb.Append("       SUM(F1.FACTOR) AS FATOR1, ");
            //stb.Append("       SUM(F2.FACTOR) AS FATOR2, ");
            stb.Append("       SUM(CAST(LEFT(R.factors, CHARINDEX(';', R.factors) - 1) AS DECIMAL(18, 6))) AS FATOR1, ");
            stb.Append("       SUM(CAST(RIGHT(R.factors, LEN(R.factors) - CHARINDEX(';', R.factors)) AS DECIMAL(18, 6))) AS FATOR2, ");
            stb.Append("       CASE ");
            ...
            stb.Append(" 	   MAX(MAXIMO_MOEDAS_CONSOLIDADO) AS MAXIMO_MOEDAS_CONSOLIDADO, ");
            stb.Append("       '' AS GRUPO, ");
            ...
            //stb.Append("LEFT JOIN GDA_FACTOR (NOLOCK) F1 ON F1.IDGDA_RESULT = R.IDGDA_RESULT ");
            //stb.Append("AND F1.[INDEX] = 1 ");
            //stb.Append("LEFT JOIN GDA_FACTOR (NOLOCK) F2 ON F2.IDGDA_RESULT = R.IDGDA_RESULT ");
            ...
            stb.Append("LEFT JOIN GDA_INDICATOR (NOLOCK) AS IT ON IT.IDGDA_INDICATOR = R.INDICADORID ");
            stb.Append("LEFT JOIN ");
            ...
            stb.Append("   WHERE CONVERT(DATE,CREATED_AT) >= @DATAINICIAL ");
            stb.Append("     AND CONVERT(DATE,CREATED_AT) <= @DATAFINAL ");
            ...
            stb.Append("LEFT JOIN ( ");
            stb.Append("SELECT (SUM(MONETIZATION)/COUNT(0)) AS MAXIMO_MOEDAS_CONSOLIDADO, INDICATOR_ID, CONVERT(DATE,STARTED_AT) AS START,  ");
            stb.Append("CONVERT(DATE,ENDED_AT) AS ENDED  FROM GDA_HISTORY_INDICATOR_GROUP (NOLOCK) ");
            stb.Append("WHERE DELETED_AT IS NULL  ");
            stb.Append("AND GROUPID = 1 AND MONETIZATION > 0 ");
            stb.Append("GROUP BY INDICATOR_ID, CONVERT(DATE,STARTED_AT), CONVERT(DATE,ENDED_AT) ");
            stb.Append(") AS MH ON MH.INDICATOR_ID = R.INDICADORID AND MH.START <= R.CREATED_AT AND MH.ENDED >= R.CREATED_AT   ");
            //FK
            ...
            //stb.Append("       MAX(BK.GANHO_MAXIMO) AS META_MAXIMA, ");
            //stb.Append("       '' AS GRUPO, ");
            //stb.Append("       GETDATE() AS 'Data de atualização', ");
            //stb.Append("       MAX(SEC.IDGDA_SECTOR) AS COD_GIP, ");
            //stb.Append("       MAX(SEC.NAME) AS SETOR, ");
            ...
            //stb.Append("AND CA.RESULT_DATE >= R.CREATED_AT "); //@DATAINICIAL
            ...
            ////stb.Append("LEFT JOIN ( ");
            ////stb.Append("		 SELECT COUNT(0)AS SSS, IDGDA_COLLABORATORS FROM GDA_RESULT (NOLOCK)  ");
            ////stb.Append("		 WHERE INDICADORID = -1  ");
            ////stb.Append("		 AND CREATED_AT >= @DATAINICIAL AND CREATED_AT <= @DATAFINAL ");
            ////stb.Append("		 AND FACTORS = '1.000000;0.000000' AND DELETED_AT IS NULL ");
            ////stb.Append("		 GROUP BY IDGDA_COLLABORATORS ");
            ////stb.Append(") AS DT ON DT.IDGDA_COLLABORATORS =  R.IDGDA_COLLABORATORS  ");
            ////stb.Append("LEFT JOIN ");
            ////stb.Append("  (SELECT collaborator_id, ");
            ////stb.Append("          GDA_INDICATOR_IDGDA_INDICATOR, ");
            ...
            ////stb.Append("            GDA_INDICATOR_IDGDA_INDICATOR) AS MZ ON MZ.GDA_INDICATOR_IDGDA_INDICATOR = R.INDICADORID ");
            ////stb.Append("AND MZ.collaborator_id = R.IDGDA_COLLABORATORS ");
            //stb.Append("LEFT JOIN ");
            //stb.Append("  (SELECT SUM(INPUT) AS SOMA, ");
            //stb.Append("          IDGDA_SECTOR, ");
            ...
            //stb.Append("AND MZ.COLLABORATOR_ID = R.IDGDA_COLLABORATORS ");
            //stb.Append(" ");
            //stb.Append("LEFT JOIN (SELECT SUM(MONETIZATION_G1) AS GANHO_MAXIMO,  ");
            //stb.Append("        SECTOR_ID, INDICATOR_ID  ");
            //stb.Append(" FROM GDA_BASKET_INDICATOR (NOLOCK)  ");
            //stb.Append(" WHERE DATE = CONVERT(DATE, DATEADD(DAY, -1, GETDATE()))  ");
            //stb.Append(" GROUP BY SECTOR_ID, INDICATOR_ID) AS BK ON BK.SECTOR_ID = HCS.IDGDA_SECTOR AND BK.INDICATOR_ID = R.INDICADORID ");
            //stb.Append("LEFT JOIN GDA_SECTOR (NOLOCK) AS SEC ON SEC.IDGDA_SECTOR = HCS.IDGDA_SECTOR ");
            //stb.Append("LEFT JOIN GDA_HISTORY_INDICATOR_SECTORS (NOLOCK) AS HIS ON HIS.INDICATOR_ID = R.INDICADORID ");
            //stb.Append("AND HIS.SECTOR_ID = HCS.IDGDA_SECTOR AND HIS.DELETED_AT IS NULL ");
            //stb.Append("LEFT JOIN GDA_COLLABORATORS_LAST_DETAILS (NOLOCK) AS CL ON CL.IDGDA_COLLABORATORS = CB.IDGDA_COLLABORATORS ");
            //stb.Append("WHERE 1 = 1 ");
            //stb.Append("  AND R.CREATED_AT >= @DATAINICIAL ");
            //stb.Append("  AND R.CREATED_AT <= @DATAFINAL ");
            ////stb.Append("  AND (BK.GANHO_MAXIMO IS NOT NULL) ");
            //stb.AppendFormat("  {0} ", filter);
            //stb.Append("GROUP BY R.IDGDA_COLLABORATORS, ");
            //stb.Append("         R.INDICADORID, ");
            //stb.Append("         CONVERT(DATE, R.CREATED_AT) ");
                ...
                using (SqlCommand command = new SqlCommand(stb.ToString(), connection))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            //if (reader["dias trabalhados"].ToString() != "")
                            //{
                            //    int diasTrab = int.Parse(reader["dias trabalhados"].ToString());
                            //    int metaMaxima = int.Parse(reader["meta maxima de moedas"].ToString());
                            //    rmam.diasTrabalhados = diasTrab.ToString();
                            //    rmam.moedasPossiveis = (diasTrab * metaMaxima);
                            //}
                            //else
                            //{
                            ...
                            DateTime dataAg = DateTime.Parse(rmam.data);
                            int idCola = int.Parse(rmam.idcollaborator);

                            int trabalhado = lColaboradoresTrabalhados.Where(t => t.data == dataAg && t.id == idCola).Select(t => t.trabalhado).DefaultIfEmpty(-1).Max();
                            if (trabalhado != -1)
                            {
                                rmam.diasTrabalhados = trabalhado.ToString();
                            }
                            else
                            {
                            ...
                            if (escalado != -1)
                            {
                                rmam.diasEscalados = escalado.ToString();
                            }
                            else
                            {
                                rmam.diasEscalados = "-";
                            ...
                            if (reader["MAXIMO_MOEDAS_CONSOLIDADO"].ToString() != "")
                            {
                                rmam.moedasPossiveisConsolidado = int.Parse(reader["MAXIMO_MOEDAS_CONSOLIDADO"].ToString());
                            }
                            else
                            {
                                //Não veio informação
                                rmam.moedasPossiveisConsolidado = 0;
                            }
        ...
        public class returnResponseADM
        {
        ...
        //    {
        //        sb.Append("  AND result_date = @DATEENV  ");
        //    }
        //    else
        //    {
        //        sb.Append("  AND result_date >= @DateEnv AND result_date <= @DATEFIM  ");
        ...
        //    sb.Append("        AND COLLABORATOR_ID = @INPUTID) AS 'GANHO EM MOEDAS',  ");
        //    sb.Append("       HIS.GOAL, ");
                ...
                if (hierarchy == "SUPERVISOR")
                {
                    retorno = original
                        ...
                        .Select(grupo => new ModelsEx.homeRel
                        {
                            ...
                            cod_gip_supervisor = grupo.First().cod_gip_supervisor,
                            ...
                            setor_supervisor = grupo.First().setor_supervisor,
                            ...
                            //moedasPossiveis = grupo.Max(item => item.moedasPossiveisConsolidado),
        ...
        public IHttpActionResult PostResultsModel([FromBody] InputModel inputModel)
        {
            ...
            List<ModelsEx.homeRel> rmams = new List<ModelsEx.homeRel>();
            rmams = returnMonetizationAdmMonth(dtInicial, dtFinal, sectorsAsString, indicatorsAsString, colaboratorsAsString, hierarchiesAsString, inputModel.Order);

            //rmams = rmams.FindAll(d => d.meta != "").ToList();
            ...
            //Filtros nas hirarquias para quando é enviado um colaborador
            ...
            //List<ModelsEx.homeRel> asdasdasdad = ceo.FindAll(it => it.idcollaborator == "781530" && it.cod_indicador == "665").ToList();

            //Regra para pegar as moedas ganhas
            for (int i = 0; i < listaHieraquias.Count; i++)
            {
                ModelsEx.homeRel hierarquia = listaHieraquias[i];

                if (hierarquia.idcollaborator == "601071" && hierarquia.cod_indicador == "4")
                {
                    var parou = true;
                }
            ...
            {
                listaCesta = Funcoes.retornaCestaIndicadores(rmams, cm, true, true, false);
            }

            List<ModelsEx.homeRel> teste = new List<ModelsEx.homeRel>();
            teste = listaCesta.FindAll(item => item.cod_indicador == "10000012" || item.idcollaborator == "78957" ).ToList();
            ...
            List<ModelsEx.homeRel> resultList = rmams.GroupBy(d => new { d.idcollaborator, d.indicador }).Select(item => new ModelsEx.homeRel
            {
                ...
                //meta = (item.Sum(item2 => Convert.ToDouble(item2.meta)) / item.Count()).ToString(),
                data_atualizacao = item.First().data_atualizacao,
                cod_gip = item.First().cargo == "SUPERVISOR" ? item.First().cod_gip_supervisor : item.First().cod_gip,
                setor = item.First().cargo == "SUPERVISOR" ? item.First().setor_supervisor : item.First().setor,
                ...
                vemMeta = item.First().vemMeta,
                peso = item.Max(item2 => item2.peso),
                diasTrabalhados = item.Sum(item2 => int.Parse(item2.diasTrabalhados.Replace("-", "0"))).ToString(),
                diasEscalados = item.Max(item2 => item2.diasEscalados),
                moedasPossiveis = item.Sum(item2 => item2.moedasPossiveis),
            ...
            //List<ModelsEx.homeRel> asdasdasdad2 = resultList.FindAll(it => it.idcollaborator == "789557").ToList();


            listaAgentes = resultList
                    .Where(item => item.cargo.ToUpper() == "AGENTE")
                        .ToList();

            listaHieraquias = resultList
                   .Where(item => item.cargo.ToUpper() != "AGENTE")
                       .ToList();

            List<ModelsEx.homeRel> asdasdasdad2 = listaHieraquias.FindAll(it => it.idcollaborator == "789587").ToList();
            ...
            for (int i = 0; i < listaHieraquias.Count; i++)
            {
                ...
                //for (DateTime dataAtual = dtTimeInicial; dataAtual <= dtTimeFinal; dataAtual = dataAtual.AddDays(1))
                //{
                //    factor = returnFactorsById(hierarquia.idgda_collaborators, dataAtual.ToString("yyyy-MM-dd"), hierarquia.codIndicador, hierarquia.cod_gip, factor, true, dtFinal);
                //}
                ...
                //factor2 = returnCoins(hierarquia.idgda_collaborators, dtInicial, hierarquia.codIndicador, hierarquia.cod_gip, factor, false, dtFinal);
                if (hierarquia.idcollaborator == "766162" && hierarquia.cod_indicador == "4")
                {
                    var parou = true;
                }
                ...
                //Realiza a conta
                hierarquia = monetizationClass.doCalculationResultHome(hierarquia, true);

                //if (factor2.coins == "")
                //{
                //    factor2.coins = "0";
                //}
                //hierarquia.moedasGanhas = double.Parse(factor2.coins);
                ////hierarquia = monetizationClass.doCalculationResultDay(hierarquia);
                //listaHieraquias[i] = hierarquia;
            }
            ...
            //if (indicatorsCestaAsString != "" || indicatorsAsString == "")
            //{
            //    listaCesta = Funcoes.retornaCestaIndicadores(rmams, cm, false, true, false);
            //}
            ...
            //List<ModelsEx.homeRel> TESS = elementosFiltrados.FindAll(it => it.idcollaborator == "781530" && it.cod_indicador == "665").ToList();

            var jsonData = elementosFiltrados.Select(item => new returnResponseADM
            {
                Mes = item.mes,
                Ano = item.ano,
                ...
                //Percentual = item.porcentual == 100.0 ? 1000 : item.porcentual,
