...
{
    //[Authorize]
    public class ReportHomeResult2Controller : ApiController
    {
                ...
                using (SqlCommand command = new SqlCommand(stb.ToString(), connection))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            //string factor0 = factors.Split(";")[0];
                            //string factor1 = factors.Split(";")[1];
        ...
        public IHttpActionResult PostResultsModel([FromBody] InputModel inputModel)
        {
            ...
            //List<ModelsEx.homeRel> testeAg = new List<ModelsEx.homeRel>();
            //testeAg = rmams.FindAll(i => i.idcollaborator == "").ToList();
            ...
            for (int i = 0; i < hierarchies.Count; i++)
            {
                try
                {
                    ...
                    if (agente.idcollaborator == "789587")
                    {
                        var parou = true;
                    }
                    ...
                    //if (agente.name == "SAMUEL JANUARIO DE SOUZA")
                    //{
                    //    var parou = true;
                    //}
            ...
            //List<ModelsEx.homeRel> asdasdasd = listaCesta.FindAll(i => i.idcollaborator == "652560").ToList();
            ...
            var jsonData = elementosFiltrados.Select(item => new returnResponseHome
            {
                ...
                //PercentualDeAtingimento = ((item.cargo == "AGENTE" || item.cargo == "SUPERVISOR") && item.vemMeta == 1) || item.cod_indicador == "10000012" ? item.porcentual.ToString() : "-",
