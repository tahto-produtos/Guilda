...
{
    //[Authorize]
    public class ResultConsolidated1Controller : ApiController
    {
        public class Sector
        {
        ...
        public List<HomeResultConsolidated> ReturnHomeResultConsolidated(string idCol, string dtinicial, string dtfinal, string hierarchies, string indicators, string sectors)
        {
            string filter = "";
            ...
            //stb.AppendFormat("       MAX(TBL.CARGO), ");
            ...
            stb.AppendFormat("               WHEN TBL.IDGDA_PERIOD = 1 THEN (HIS.GOAL) ");
            stb.AppendFormat("               WHEN TBL.IDGDA_PERIOD = 2 THEN (HIS.GOAL_NIGHT) ");
            stb.AppendFormat("               WHEN TBL.IDGDA_PERIOD = 3 THEN (HIS.GOAL_LATENIGHT) ");
            stb.AppendFormat("               ELSE 0 ");
            ...
            stb.AppendFormat("	   MAX(TBL.MOEDA_GANHA) AS MOEDA_GANHA ");
            ...
            stb.AppendFormat("	   MAX(MZ.INPUT) AS MOEDA_GANHA ");
            ...
            stb.AppendFormat("     AND CREATED_AT <= @DATAFINAL ) AS CL ON CL.IDGDA_COLLABORATORS = CB.IDGDA_COLLABORATORS ");
            stb.AppendFormat("AND CL.CREATED_AT = R.CREATED_AT ");
            stb.AppendFormat("INNER JOIN GDA_INDICATOR (NOLOCK) AS IT ON IT.IDGDA_INDICATOR = R.INDICADORID ");
            stb.AppendFormat(" ");
            stb.AppendFormat("LEFT JOIN ");
            ...
            stb.AppendFormat("  AND CL.IDGDA_SECTOR IS NOT NULL ");
            ...
            stb.AppendFormat("  AND R.FACTORS <> '0.000000; 0.000000' ");
            stb.AppendFormat("  {0}  ", filter);
            stb.AppendFormat("GROUP BY R.INDICADORID, CL.IDGDA_SECTOR, ");
            ...
            stb.AppendFormat("AND HIG1.SECTOR_ID = TBL.IDGDA_SECTOR ");
            ...
            stb.AppendFormat("AND HIG2.SECTOR_ID = TBL.IDGDA_SECTOR ");
            ...
            stb.AppendFormat("AND HIG3.SECTOR_ID = TBL.IDGDA_SECTOR ");
            ...
            stb.AppendFormat("AND HIG4.SECTOR_ID = TBL.IDGDA_SECTOR ");
        ...
        public IHttpActionResult PostResultsModel([FromBody] InputModel inputModel)
        {
            ...
            int contadorDima = jsonData.Count(item => item.IDGRUPO == 1 && item.MONETIZATION);
            int contadorOuro = jsonData.Count(item => item.IDGRUPO == 2 && item.MONETIZATION);
            int contadorPrata = jsonData.Count(item => item.IDGRUPO == 3 && item.MONETIZATION);
            int contadorBronze = jsonData.Count(item => item.IDGRUPO == 4 && item.MONETIZATION);

            int totalElementos = jsonData.Count(item => item.MONETIZATION);
