...
{
    //[Authorize]
    public class EditQuizAnswerController : ApiController
    {// POST: api/Results
        ...
        public IHttpActionResult PostResultsModel([FromBody] InputModel inputModel)
        {
            ...
            string idLog = Logs.InsertActionLogs("Atualizar resposta Quiz ", "GDA_QUIZ_ANSWER", COLLABORATORID.ToString());
            int IDGDA_QUIZ_ANSWER = inputModel.IDGDA_QUIZ_ANSWER;
            string QUESTION = inputModel.QUESTION;
            int RIGHT_ANSWER = inputModel.RIGHT_ANSWER;

            #region UPDATE GDA_QUIZ_ANSWER
            StringBuilder InsertNotification = new StringBuilder();
            InsertNotification.Append("UPDATE GDA_QUIZ_ANSWER SET  ");
            InsertNotification.Append($"QUESTION = '{QUESTION}', RIGHT_ANSWER = '{RIGHT_ANSWER}'  ");
            ...
            using (SqlConnection connection = new SqlConnection(Database.retornaConn()))
            {
                ...
                {
                    return BadRequest($"Erro ao Atualizar resposta GDA_QUIZ_ANSWER: {ex.Message}");
                }
            }
            #endregion
            return Ok("Resposta do Quiz atualizada com sucesso.");
