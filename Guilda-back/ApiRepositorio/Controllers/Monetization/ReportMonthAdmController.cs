...
{
    //[Authorize]
    public class ReportMonthAdmController : ApiController
    {
        public static List<ModelsEx.homeRel> returnMonetizationAdmMonth(string dtInicial, string dtFinal, string sectors, string indicators, string collaborators, string hierarchies, string ordem, bool Thread = false)
        {
            ...
            //CONSULTA NO BANCO DO RELATORIO DE MONETIZAÇÃO MENSAL.
            ...
            stb.Append("	   CASE  ");
            stb.Append("	       WHEN MAX(MZ.EXPIRED) IS NULL THEN 0 ");
            stb.Append("	       ELSE MAX(MZ.EXPIRED) ");
            stb.Append("	   END AS MOEDA_EXPIRADA, ");
            ...
            stb.Append("          SUM(COIN_EXPIRED) AS EXPIRED,  ");
                ...
                using (SqlCommand command = new SqlCommand(stb.ToString(), connection))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            rmam.MoedasExpiradas = reader["MOEDA_EXPIRADA"].ToString() != "" ? double.Parse(reader["MOEDA_EXPIRADA"].ToString()) : 0;
                            rmams.Add(rmam);
                        }
                    }
                }
                ...
                if (hierarchy == "SUPERVISOR")
                {
                    retorno = original
                        ...
                        .Select(grupo => new ModelsEx.homeRel
                        {
                            ...
                            sumDiasLogadosEscalados = grupo.Count(item => item.sumDiasEscalados == 1 && item.sumDiasLogados == 1),
        ...
        // POST: api/Results
        public static List<returnResponseADM> relMonMensal(string dtInicial, string dtFinal, string groupsAsString, string sectorsAsString, string hierarchiesAsString, string indicatorsAsString, string indicatorsCestaAsString, string colaboratorsAsString, string orderFil, string indicatorsAccessAsString, bool Thread = false)
        {
            ...
            List<ModelsEx.homeRel> rmams = new List<ModelsEx.homeRel>();
            rmams = returnMonetizationAdmMonth(dtInicial, dtFinal, sectorsAsString, indicatorsAsString, colaboratorsAsString, hierarchiesAsString, orderFil, Thread);

            //Adicionando os indicadores de taxa de acesso
            if (indicatorsAccessAsString != "" || (indicatorsAsString == "" && indicatorsCestaAsString == ""))
            {
                List<ModelsEx.homeRel> listaIndicadorAcesso = new List<ModelsEx.homeRel>();
                listaIndicadorAcesso = Funcoes.retornaIndicadorAcesso(dtInicial, dtFinal, sectorsAsString, colaboratorsAsString, hierarchiesAsString, orderFil, Thread);
                listaIndicadorAcesso = listaIndicadorAcesso.FindAll(item => item.moedasPossiveis > 0).ToList();
            ...
            //REGRA PARA PEGAR AS MOEDAS GANHAS HIERARQUIA ACIMA DE AGENTE
            ...
            teste = listaCesta.FindAll(item => item.cod_indicador == "10000013").ToList();
            ...
            List<ModelsEx.homeRel> resultList = rmams.GroupBy(d => new { d.idcollaborator, d.indicador }).Select(item => new ModelsEx.homeRel
            {
                ...
                MoedasExpiradas = item.Sum(item2 => item2.MoedasExpiradas),
                ...
                sumDiasEscalados = item.Sum(item2 => item2.sumDiasEscalados),
                sumDiasLogados = item.Sum(item2 => item2.sumDiasLogados),
                sumDiasLogadosEscalados = item.Sum(item2 => item2.sumDiasLogadosEscalados),
            ...
            var jsonData = elementosFiltrados.Select(item => new returnResponseADM
            {
                ...
                MoedasExpiradas = item.MoedasExpiradas,
        ...
        public IHttpActionResult PostResultsModel([FromBody] InputModel inputModel)
        {
            ...
            string indicatorsAsString = string.Join(",", inputModel.Indicators.Where(i => i.Id.ToString() != "10000012" && i.Id.ToString() != "10000013").Select(g => g.Id));
            ...
            string indicatorsAcessoAsString = string.Join(",", inputModel.Indicators.Where(i => i.Id.ToString() == "10000013").Select(g => g.Id));
            ...
            var jsonData = relMonMensal(dtInicial, dtFinal, groupsAsString, sectorsAsString, hierarchiesAsString, indicatorsAsString, indicatorsCestaAsString, colaboratorsAsString, order, indicatorsAcessoAsString);
