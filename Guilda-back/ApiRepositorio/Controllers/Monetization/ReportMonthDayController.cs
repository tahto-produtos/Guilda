...
{
    //[Authorize]
    public class ReportMonthDayController : ApiController
    {
        public static List<ModelsEx.homeRel> returnMonetizationDayMonth(string dtInicial, string dtFinal, string sectors, string indicators, string collaborators, string hierarchies, string ordem, bool Thread = false)
        {
            ...
            //stb.Append("IDGDA_COLLABORATOR, CONVERT(DATE, DATE_ACCESS) AS CREATED_AT ");
            //stb.Append("FROM GDA_LOGIN_ACCESS (NOLOCK) ");
            //stb.Append("WHERE CONVERT(DATE, DATE_ACCESS) >= @DATAINICIAL  ");
            //stb.Append("AND CONVERT(DATE, DATE_ACCESS) <= @DATAFINAL   ");
            //stb.Append("GROUP BY IDGDA_COLLABORATOR, CONVERT(DATE, DATE_ACCESS)) AS LOG ON LOG.IDGDA_COLLABORATOR = R.IDGDA_COLLABORATORS ");
            //stb.Append("AND LOG.CREATED_AT = R.CREATED_AT ");
            ...
            stb.Append("LEFT JOIN ");
            stb.Append("  (SELECT IDGDA_SECTOR, ");
        ...
        public static List<returnResponseDay> relMonDiario(string dtInicial, string dtFinal, string groupsAsString, string sectorsAsString, string hierarchiesAsString, string indicatorsAsString, string indicatorsCestaAsString, string colaboratorsAsString, string orderFil, string indicatorsAccessAsString, string indicatorsTaxaAcessoAsString, bool Thread = false)
        {

            Funcoes.cestaMetrica cm = Funcoes.getInfMetricBasket(Thread);

            //List<Funcoes.indicatorAccessMetrica> cia = Funcoes.getInfMetricAccess(Thread, dtInicial, dtFinal);
            ...
            if (indicatorsAccessAsString != "" || indicatorsTaxaAcessoAsString != "" || (indicatorsAsString == "" && indicatorsCestaAsString == ""))
            {
            ...
            //FILTRO DA INDICADOR DE ACESSO.
            //List<ModelsEx.homeRel> listaIndicadorAcesso = new List<ModelsEx.homeRel>();
            //if (indicatorsAcessoAsString != "" || indicatorsAcessoAsString == "")
            //{
            //    listaIndicadorAcesso = Funcoes.retornaIndicadorAcesso(rmams, cia, true, false, false);
            //}


            if (indicatorsAsString != "" || indicatorsTaxaAcessoAsString != "" || indicatorsAccessAsString != "")
            {
                ...
                else if (indicatorsAccessAsString != "")
                {
                    List<string> valoresLista = indicatorsAccessAsString.Split(',').Select(valor => valor.Trim()).ToList();
                    rmams = rmams.FindAll(item => valoresLista.Contains(item.cod_indicador)).ToList();
                }
                else if (indicatorsTaxaAcessoAsString != "")
                {
                    List<string> valoresLista = indicatorsTaxaAcessoAsString.Split(',').Select(valor => valor.Trim()).ToList();
                    rmams = rmams.FindAll(item => valoresLista.Contains(item.cod_indicador)).ToList();
                }
            ...
            //FILTRO DA INDICADOR DE ACESSO.
            //if ((indicatorsCestaAsString != "" || indicatorsAcessoAsString != "") && indicatorsAsString == "")
            //{
            //    rmams.Clear();
            //    if (indicatorsCestaAsString != "")
            //    {

            //        rmams = rmams.Concat(listaCesta).ToList();
            //    }
            //    if (indicatorsAcessoAsString != "")
            //    {
            //        rmams = rmams.Concat(listaIndicadorAcesso).ToList();
            //    }
            //}
            //else if (indicatorsCestaAsString != "" || indicatorsAcessoAsString != "")
            //{
            //    if (indicatorsCestaAsString != "")
            //    {
            //        rmams = rmams.Concat(listaCesta).ToList();
            ...
            if (indicatorsCestaAsString != "" && indicatorsAsString == "" && indicatorsAccessAsString == "" && indicatorsTaxaAcessoAsString == "")
            {
                rmams.Clear();
                rmams = rmams.Concat(listaCesta).ToList();
            }
            ...
            else if (indicatorsAsString == "" && indicatorsAccessAsString == "" && indicatorsTaxaAcessoAsString == "")
            {
                rmams = rmams.Concat(listaCesta).ToList();
            }
            ...
            var jsonData = elementosFiltrados.Select(item => new returnResponseDay
            {
                ...
                fator0 = item.fator0,
                fator1 = item.fator1,
                fatores = item.fator0 + "" + item.fator1,
                idGrupo = item.grupoNum,
                aliasGrupo = item.grupoAlias,
        ...
        public IHttpActionResult PostResultsModel([FromBody] InputModel inputModel)
        {
            ...
            string indicatorsAsString = string.Join(",", inputModel.Indicators.Where(i => i.Id.ToString() != "10000012" && i.Id.ToString() != "10000013" && i.Id.ToString() != "10000014").Select(g => g.Id));
            ...
            string indicatorsTaxaAcessoAsString = string.Join(",", inputModel.Indicators.Where(i => i.Id.ToString() == "10000014").Select(g => g.Id));
            ...
            var jsonData = relMonDiario(dtInicial, dtFinal, groupsAsString, sectorsAsString, hierarchiesAsString, indicatorsAsString, indicatorsCestaAsString, colaboratorsAsString, order, indicatorsAcessoAsString, indicatorsTaxaAcessoAsString);
