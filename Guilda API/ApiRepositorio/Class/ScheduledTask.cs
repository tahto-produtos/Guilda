...
namespace ApiRepositorio.Class
{
    public class ScheduledTask
    {
        ...
        {
            Log.insertLogTransaction("", "SEMANAL", "Inicia processo semanal", "");
            try
            {
                processoSemanalMensal();
                Log.insertLogTransaction("", "SEMANAL", "Finaliza processo semanal", "");
            }
            catch (Exception ex)
            {
                Log.insertLogTransaction("", "SEMANAL", $"Execute Error: {ex.Message}", "");
            }
        ...
        public static void processoSemanalMensal()
        {
            ...
            //Mes anterior
            //1ª Semana
            //Log.insertLogTransaction("", "SEMANAL", "Inicia processo semanal -4", "");
            ...
            //Mes anterior
            //1ª Semana
            //Log.insertLogTransaction("", "SEMANAL", "Inicia processo semanal -3", "");
            ...
            //Mes anterior
            //1ª Semana
            //Log.insertLogTransaction("", "SEMANAL", "Inicia processo semanal -2", "");
            ...
            //Mes anterior
            //1ª Semana
            Log.insertLogTransaction("", "SEMANAL", "Inicia processo semanal -1", "");
            ...
            //Mes Atual
            //1ª Semana
            Log.insertLogTransaction("", "SEMANAL", "Inicia processo semanal 0", "");
            ...
            try
            {
                ...
                List<ResultsConsolidatedController.usuariosBk> users = ResultsConsolidatedController.returnUsers(dtAg);
        ...
        public static List<semanalMensalConsolidado> retornaCesta(List<semanalMensalConsolidado> rmams)
        {
            List<groups> lgroup = listGroups("");
            ...
            cii = rmams.GroupBy(d => new { d.MATRICULA, d.DATAPAGAMENTO }).Select(item => new semanalMensalConsolidado
            {
                MATRICULA = item.Key.MATRICULA,
                CARGO = item.First().CARGO,
                //CODGIP = item.First().CODGIP,
                //SETOR = item.First().SETOR,
                DATAPAGAMENTO = item.Key.DATAPAGAMENTO,
            ...
            cestaMetrica cm = getInfMetricBasket();
        ...
        public static List<returnResponseSemanalMensal> ConsolidatedResultSemanalDiario(int semana, int mes, int ano)
        {
            ...
            if (semana == 0)
            {
                ...
                List<semanalMensalConsolidado> cii = new List<semanalMensalConsolidado>();
                cii = retornaCesta(rmams);

                cii = cii.Concat(rmams).ToList();
                Log.insertLogTransaction("", "SEMANAL", "Insere performance", "");
                inserirInfBanco(cii, semana, dia, mes, ano, true, true);
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(Database.retornaConn()))
                {
                    if (diario == true)
                    {
                        ...
                        colunas = colunas.Replace("[DATAPAGAMENTO] VARCHAR(MAX)", "[DATAPAGAMENTO] DATETIME");
                        connection.Open();
                        ...
                        using (SqlBulkCopy bulkCopy = new SqlBulkCopy(connection))
                        {
                            ...
                            //bulkCopy.NotifyAfter = 1000; // Notifica a cada 1000 linhas copiadas, por exemplo
                        ...
                        try
                        {
                            ...
                            stb.AppendFormat("CONVERT(VARCHAR(10), DATAPAGAMENTO, 23),"); //DATA, 
                        ...
                        {
                            Log.insertLogTransaction("", "SEMANAL", ex.Message, "");
                        }
        ...
        public static List<semanalMensalConsolidado> ReturnHomeResultConsolidated(string dtinicial, string dtfinal)
        {
            ...
            stb.AppendFormat($"LEFT JOIN GDA_SECTOR (NOLOCK) AS SUBSEC ON SEC.IDGDA_SECTOR = TBL.IDGDA_SUBSECTOR "); //WHERE TBL.IDGDA_COLLABORATORS = '647584'
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            try
                            {
                                ...
                                hrc.META_MAXIMA_MOEDAS = reader["QTD_MON_TOTAL"].ToString() != "" ? double.Parse(reader["QTD_MON_TOTAL"].ToString()) : 0;
        ...
        public static List<semanalMensalConsolidado> ReturnHomeResultConsolidatedAccess(string dtinicial, string dtfinal, string indicatorsAsString)
        {
            ...
            stb.AppendFormat("AND ME.DELETED_AT IS NULL ");
            stb.AppendFormat($"LEFT JOIN GDA_SECTOR (NOLOCK) AS SEC ON SEC.IDGDA_SECTOR = TBL.IDGDA_SECTOR ");
            stb.AppendFormat($"LEFT JOIN GDA_SECTOR (NOLOCK) AS SECSUB ON SECSUB.IDGDA_SECTOR = TBL.IDGDA_SUBSECTOR "); //WHERE TBL.MATRICULA_SUPERVISOR = '717829'
