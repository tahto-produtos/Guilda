...
import { ConfigDisplay } from "src/pages/marketing-place/display-config";
import { UpdateDisplayUseCase } from "../use-cases/update-display.use-case";
import { ListProductsByVisibility } from "../use-cases/list-product-by-visibility.use-case";

interface ModalExportResultsProps {
    open: boolean;
    onClose: () => void;
    displayToEdit: ConfigDisplay;
}
...
export function EditDisplayModal(props: ModalExportResultsProps) {
    const { onClose, open, displayToEdit } = props;

    const { finishLoading, isLoading, startLoading } = useLoadingState();

    const [name, setName] = useState<string>("");
    const [priority, setPriority] = useState<string>("1");
    const [status, setStatus] = useState<string>(displayToEdit.STATUS);
    ...
    const [stockAdd, setStockAdd] = useState<boolean>(false);
    ...
    useEffect(() => {
        setName(displayToEdit.NOMECONFIG);
        setPriority(displayToEdit.PRIORIDADE);
        const foundStock = stocks.find(
            (item) => item.id == displayToEdit.ESTOQUE[0].CODESTOQUE
        );
        if (!stockAdd) {
            if (stocks && foundStock) {
                setStock(foundStock);
                setStockAdd(true);
            }
        }
        if (!productAdd && products) {
            setProduct(
                displayToEdit.ITENS.map((item) => {
                    return {
        ...
        setGroup(
            displayToEdit.GRUPO.map((item) => {
                const groupName =
                    item.NOMEGRUPO == "DIAMANTE"
                        ? "G1"
                        : item.NOMEGRUPO == "OURO"
                        ? "G2"
                        : item.NOMEGRUPO == "PRATA"
                        ? "G3"
                        : "G4";
        ...
        setHierarchie(
            displayToEdit.HIERARQUIA.map((item) => {
                return {
                    id: item.CODHIERARQUIA,
                    levelName: item.NOMEHIERARQUIA,
                };
            }) as Hierarchie[]
        );
    }, [displayToEdit, stocks]);
    ...
    async function handleCreate() {
        if (product.length <= 0 || !stock) {
            return toast.warning(
                "Escolha um estoque e adicione produtos para criar uma vtirine."
            );
        }
        startLoading();
        ...
        new UpdateDisplayUseCase()
            .handle(payload, displayToEdit.IDCONFIG, status)
            .then((data) => {
                toast.success("Vitrine editada com sucesso!");
                onClose();
            })
            .catch((data) => {
