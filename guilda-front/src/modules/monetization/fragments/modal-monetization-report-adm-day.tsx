...
import { Group, Indicator, Sector, SectorAndSubsector } from "src/typings";
...
import { ExportMonetizationAdmDayReportUseCase } from "../use-cases/export-report-adm-monetization-day.use-case";
...
import { ListSectorsAndSubsectrosUseCase } from "src/modules/sectors/use-cases/list-sector-and-subsector.use-case";
...
export function ModalMonetizationReportAdmDay(props: ModalExportResultsProps) {
...
/*         const pagination: PaginationModel = {
    ...
    async function handleReportExtract() {
        ...
        new ExportMonetizationAdmDayReportUseCase()
            .handle(payload)
            .then((data) => {
                if (data.length <= 0) {
                ...
                const docRows = data.map((item: any) => {
                    return [
                        item.DataPagamento
                            ? formatServerDate(item.DataPagamento)
                            : "",
                        ...
                        item.Cargo,
                        item.DataReferencia
                            ? formatServerDate(item.DataReferencia)
                            : "",
                            { v: item.IDIndicador, t: "n" },
                ...
                indicatorSheetBuilder
                    ...
                    .append(docRows)
                    .exportAs(`Relatório Analítico Monetização Diário`);
    ...
    return (
        <BaseModal
            width={"540px"}
            open={open}
            title={`Relatório Analítico Monetização Diário`}
            onClose={onClose}
        >
