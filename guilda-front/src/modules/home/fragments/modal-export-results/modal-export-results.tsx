...
import { ExportResultsReportUseCase } from "../../use-cases/export-results-report";
...
export function ModalExportResults(props: ModalExportResultsProps) {
...
/*                         item.Resultado && !isNaN(parseInt(item.Resultado))
                            ? {
                                v:
                                    item.TipoIndicador === "INTEGER" || item.TipoIndicador === "HOUR"
                                        ? parseInt(item.Resultado) // Se for inteiro ou hora, exportar como número inteiro
                                        : parseFloat(item.Resultado.replace(",", ".")), // Se for decimal, exportar como número decimal
                                t: "n", // Definido como número
                                z: item.TipoIndicador === "PERCENT" ? "0.00%" : "0.00", // Definir o formato no Excel (percentual ou número)
                            }
                            : "", // Caso o Resultado não seja válido, exportar como string vazia */
...
/*                         item.Meta
                            ? {
                                v: parseFloat(item.Meta.replace(",", ".")), // Converter para número
                                t: "n", // Definido como número
                                z: item.TipoIndicador === "PERCENT" ? "0.00%" : "0.00", // Definir o formato no Excel (percentual ou número)
                            }
                            : "", // Caso a Meta não exista, exportar como string vazia */
...
/*                         item.PercentualDeAtingimento && !isNaN(parseFloat(item.PercentualDeAtingimento))
                            ? {
                                v: parseFloat(item.PercentualDeAtingimento) / 100, // Dividir por 100 para ajustar ao formato percentual do Excel
                                t: "n", // Definido como número
                                z: "0.00%", // Definir o formato de percentagem com duas casas decimais
                            }
                            : "", // Caso o valor não seja válido, exportar como string vazia */
