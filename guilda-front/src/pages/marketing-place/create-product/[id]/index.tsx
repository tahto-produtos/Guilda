...
export default function CopyAndCreateProduct() {
    ...
    async function getProductHistory() {
        ...
        await new DetailsProductUseCase()
            .handle(payload)
            .then((data) => {
                ...
                data.publicationDate &&
                    setPublicationDate(new Date(data.publicationDate));
                data.expirationDate &&
                    setExpirationDate(new Date(data.expirationDate));
                data.validityDate &&
                    setValidityDate(new Date(data.validityDate));
                setProductType(data.type);
                setStatus(data.productStatusId == 0 ? "Inativo" : "Ativo");
                data?.productImages &&
                    setImageHistory(
                        data.productImages.map((image: { uploadId: number }) =>
...
CopyAndCreateProduct.getLayout = getLayout("private");
